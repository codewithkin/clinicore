...
export default function NewAppointmentModal({ open, onClose, onCreate, organizationId }: Props) {
    ...
    useEffect(() => {
        if (open && organizationId) {
            ...
            const patientsPromise = fetch(`/api/patients?organizationId=${organizationId}`)
                .then(res => res.json())
                .then(data => setPatients(data.patients || []))
                .catch(err => console.error("Failed to fetch patients:", err));

            const settingsPromise = fetch(`/api/settings?organizationId=${organizationId}`)
                .then(res => res.json())
                .then(data => {
            ...
            const orgPromise = fetch(`/api/organizations/${organizationId}`)
                .then(res => res.json())
                .then(data => {
            ...
            Promise.all([patientsPromise, settingsPromise, orgPromise])
