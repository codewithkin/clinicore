        ...
        <div className="space-y-6">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                ...
                <TabsContent value="billing" className="space-y-6">
                    ...
                    {/* Invoices */}
                    <Collapsible open={openCards.billingHistory} onOpenChange={() => toggleCard('billingHistory')}>
                        <Card className="border-gray-200 rounded-2xl">
                            <CollapsibleTrigger asChild>
                                <CardHeader className="cursor-pointer hover:bg-gray-50 transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <CardDescription className="text-sm text-gray-500 mt-1">
                                                Download past invoices and payment records
                                            </CardDescription>
                                        </div>
                                        <ChevronDown className={`h-5 w-5 text-gray-500 transition-transform ${openCards.billingHistory ? 'rotate-180' : ''}`} />
                                    </div>
                                </CardHeader>
                            </CollapsibleTrigger>
                            <CollapsibleContent>
                                            ...
                                            {orders.slice(0, 5).map((order: any, idx: number) => (
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-10 h-10 ${order.status === 'paid' ? 'bg-green-100' : 'bg-yellow-100'
                                                            } rounded-lg flex items-center justify-center`}>
                                                            <Check className={`h-5 w-5 ${order.status === 'paid' ? 'text-green-600' : 'text-yellow-600'
                                                                }`} />
                                                        </div>
                                                        <div>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <Badge className={order.status === 'paid' ? "bg-green-600 text-white" : "bg-yellow-600 text-white"}>
                                                            {order.status}
                                                        </Badge>
                                                        {order.invoice_url && (
                                                            <Button
